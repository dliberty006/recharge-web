        ...
        <div style="margin-top: 15px">
          <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
            ...
            <el-form-item label="状态：">
              <el-select v-model="listQuery.signupStatus" placeholder="请选择">
                <el-option
    ...
    <div class="table-container">
      <el-table ref="signupTable"
        ...
        <el-table-column label="主题" align="center">
          <template slot-scope="scope">{{scope.row.signupTitle}}</template>
        </el-table-column>
        ...
        <el-table-column label="每日投票次数" width="100" align="center">
          <template slot-scope="scope">{{scope.row.everydaySignupNum}}</template>
        </el-table-column>
        <el-table-column label="是否重复投票"  align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.isRepeatSignup == 0">否</span>
            <span v-if="scope.row.isRepeatSignup == 1">是</span>
          </template>
        </el-table-column>
        <el-table-column label="状态"  align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.signupStatus == 10">已保存</span>
            <span v-if="scope.row.signupStatus == 20">已生效</span>
            <span v-if="scope.row.signupStatus == 30">已失效</span>
          </template>
        </el-table-column>
...
<script>
  import {fetchList, deleteSignup} from '@/api/signup'
  export default {
    name: 'signupList',
    data() {
      return {
    ...
    methods: {
      ...
      handleUpdate(index, row) {
        this.$router.push({path: '/pms/updateSignup', query: {id: row.id}})
      },
      handleDelete(index, row) {
        this.$confirm('是否要删除该活动', '提示', {
          ...
          deleteSignup(row.id).then(response => {
            this.$message({
              message: '删除成功',
              type: 'success',
              duration: 1000
      ...
      addSignup() {
        this.$router.push({path: '/pms/addSignup'})
      }
    }
  }
</script>
